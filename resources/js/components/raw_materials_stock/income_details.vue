<template>
    <StockHeader />
                ...
                <div class="_overflow _table_div">
                    <table class="_table">
                        ...
                        >
                            ...
                            <td><div v-if="currency_id == 1">{{product.price * product.count}}</div></td>
                            <td><div v-if="currency_id == 2">{{product.price}}</div></td>
                            <td><div v-if="currency_id == 2">{{product.price * product.count}}</div></td>
            ...
            >
                ...
                <Form ref="formValidate" :label-width="180">
                    ...
                    <FormItem label="* Махсулот номи" class="ivu-mb">
                        <Select v-model="newProduct.product_id" placeholder="Махсулотни танланг...">
            ...
            >
                ...
                <Form ref="formValidate" :label-width="180">
<!--                    <FormItem label="* Махсулот номи" class="ivu-mb">-->
<!--                        <Select v-model="oldProduct.product_id" placeholder="Махсулотни танланг...">-->
<!--                            <Option v-for="product in products" :value="product.id" :key="product.id">{{ product.name + " (" + product.country + ")" }}</Option>-->
<!--                        </Select>-->
<!--                        <Text :class="{ 'd-none': !product_id_err }" type="danger">Тўлдириш мажбурий!</Text>-->
<!--                    </FormItem>-->
<!--                    <FormItem label="Махсулот номи" class="ivu-mb text-dark">-->
<!--                        <Input v-model="oldProduct.name" disabled class=""></Input>-->

<!--                    </FormItem>-->
<!--                    <Text type="secondary">{{oldProduct.name}}</Text>-->
                    <FormItem label="* Нархи" class="ivu-mb">
                        <Input v-model="oldProduct.price" placeholder="Махсулот Нархи"></Input>
                        <Text :class="{ 'd-none': !product_price_err }" type="danger">Тўлдириш мажбурий!</Text>
                    </FormItem>
                    <FormItem label="* Тўп" class="ivu-mb">
                        <Input v-model="oldProduct.packageCount" placeholder="Махсулот Тўп"></Input>
                        <Text :class="{ 'd-none': !product_packageCount_err }" type="danger">Тўлдириш мажбурий!</Text>
                    </FormItem>
                    <FormItem label="* Миқдор" class="mb-0">
...
export default {
    ...
    methods : {
        ...
        async editProduct () {
            ...
            const res = await this.callApi('post', '/app/edit_income_product', this.oldProduct)

            if(res.status === 200) {

                this.incomeProducts[this.rowIndex].name         = res.data.name
                this.incomeProducts[this.rowIndex].product_id   = res.data.product_id
                this.incomeProducts[this.rowIndex].packageCount = res.data.packageCount
                this.incomeProducts[this.rowIndex].count        = res.data.count
                this.incomeProducts[this.rowIndex].price        = res.data.price
                this.incomeProducts[this.rowIndex].updated_at   = res.data.updated_at
    ...
    async created(){
        ...
        const incomeProducts    = await this.callApi('get', '/app/get_contract_products/' + this.contract_id)
